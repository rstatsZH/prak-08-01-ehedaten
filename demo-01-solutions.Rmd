...
ehedaten_tidy <- ehedaten %>% 
  ...
  ## Teil 2 Start
  mutate(datum_neu = as_date(datum)) %>% 
  ## Zeige wie Datum zu Datum Neu wird
  ## Gehe auf die "3825 failed to parse" ein, dann filter raus
  filter(!is.na(datum_neu)) %>% 
  ## Teil 2 Ende: Mache eine Histogram
  
  ## Teil 3 Start: Filter vor 1900
  filter(datum_neu < "1900-01-01") %>% 
  mutate(jahr = year(datum_neu),
         monat = month(datum_neu, label = TRUE, abbr = FALSE, locale = "de_CH.UTF-8"),
         tag = day(datum_neu),
         wochentag = wday(datum_neu, label = TRUE, abbr = FALSE, locale = "de_CH.UTF-8")) 
