...
ehedaten_tidy <- ehedaten %>% 
  ...
  # filter(is.na(datum_neu)) %>%

  ## Teil 2 Datum überschreiben
  mutate(datum = lubridate::as_date(datum)) %>% 
  filter(!is.na(datum)) %>% 
  
  ## Teil 2: Mache ein Histogram
...
# Der standard ist 30 Bins. Die Daten werden in 30 bins unterteilt und dann die Häufigkeit in jeder gezählt.
# Das sind hier alle 10 Jahre weil die Daten 300 Jahre abdecken.
# Möchten wir ein Histogram, welches die Häufigkeit pro Jahr zählt, dann wählen wir 300
...
## Schritt 8 - Group - Verbundenes Streudiagram
ehedaten_tidy %>% 
  count(monat, wochentag) %>% 
