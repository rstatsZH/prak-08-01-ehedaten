...
ehedaten_tidy <- ehedaten %>% 
  ...
  ## Teil 3 Start 
  mutate(datum2 = parse_date(entstehungszeitraum, format = "%Y.%m.%d")) %>% # strptime Funktion aus base R
  mutate(
    jahr = year(datum),
...
# Erklärung für nicht angezeigte Bars
#ehedaten_tidy %>% 
#  ggplot(aes(x = datum2)) +
#  geom_bar(fill = "red", size = 3)

# Hier liegen welche in 2017. Warum?
ehedaten_tidy %>% 
  ggplot(aes(x = datum)) +
  geom_histogram()
...
# Zurück zu Transfomieren

# Nach Jahr
ehedaten_tidy %>% 
  ggplot(aes(x = datum2)) +
  geom_histogram(bins = 277) +
  #scale_x_date(breaks = "20 years") +
  coord_flip()
